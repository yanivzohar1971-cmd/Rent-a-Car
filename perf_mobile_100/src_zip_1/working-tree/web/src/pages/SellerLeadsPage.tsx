...
import { fetchLeadsForSeller, updateLeadStatus } from '../api/leadsApi';
...
export default function SellerLeadsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError(null);
      try {
        const loadedLeads = await fetchLeadsForSeller(firebaseUser.uid);
  ...
  if (isLoading) {
    return (
      <div className="seller-leads-page">
      ...
      <div className="page-container">
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">כאן תוכל לראות פניות שקיבלת על מודעות הרכב שלך, ולעדכן סטטוס לכל ליד</p>
          </div>
        ...
        {leads.length === 0 ? (
          <div className="empty-state">
            <p>עדיין אין לך לידים</p>
            <p className="empty-state-subtitle">
              כאשר לקוחות ימלאו את טופס יצירת הקשר בעמודי הרכבים שלך, הפניות יופיעו כאן.
            </p>
            <Link to="/seller/account" className="btn btn-primary">
              לצפייה במודעות שלך
            </Link>
          </div>
        ) : (
